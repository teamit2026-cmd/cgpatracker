class BackupService {
  ...
  async backupUserToMongo(userData) {
    try {
      ...
      console.log('Backing up user to MongoDB:', userData.email);
  ...
  async syncAllData() {
    try {
      ...
      console.log('Starting full data sync with MongoDB');
  ...
  async restoreFromMongo(userId) {
    try {
      ...
      console.log('Restoring data from MongoDB for user:', userId);

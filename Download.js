...
const Download = () => {
  ...
  const exportToPDF = async () => {
    ...
    try {
      ...
      const htmlContent = `
        ...
        const code = Array.isArray(subject) ? subject[0] : (subject.code || 'N/A');
        const name = Array.isArray(subject) ? subject[1] : (subject.name || 'N/A');
        ...
        return `
          ...
          <div class="footer">
            <div style="margin-top: 10px;">Generated by PKIET CGPA Tracker App on ${new Date().toLocaleString()}</div>
            <br />
            <div class="developed-by">Developed by</div>
            <div class="team-hexonyx">Team <span class="hex-highlight">Hexonyx</span></div>
          </div>
        </body>
        </html>
      `;
  ...
  const uniqueDepartments = React.useMemo(() => {
    // Filter out custom subjects from standard department tabs
    const depts = [...new Set(results.filter(r => !r.isCustom).map(r => r.department).filter(Boolean))];
  ...
  const tabs = React.useMemo(() => {
    ...
    // 1. Add standard department tabs
    const sortedDepts = [...uniqueDepartments].sort((a, b) => {
      if (a === userDept) return -1;
      if (b === userDept) return 1;
      return a.localeCompare(b);
    });

    sortedDepts.forEach(dept => {
      tabsArray.push({ id: dept, name: dept, isCustom: false });
    });

    // 2. Add "Custom Subjects" tab if custom results exist
    const hasCustomResults = results.some(r => r.isCustom);
    if (hasCustomResults) {
      tabsArray.push({ id: 'custom_subjects_tab', name: 'Custom Subjects', isCustom: true });
    }

    return tabsArray;
  }, [uniqueDepartments, currentUser, results]);

  // Get results for a specific tab
  const getResultsForTab = (tab) => {
    if (!tab) return [];
    if (tab.isCustom) {
      return results.filter(r => r.isCustom);
    }
    return results.filter(r => !r.isCustom && r.department === tab.id);
  };
  ...
  const currentTabResults = React.useMemo(() => {
    return getResultsForTab(tabs[currentTab]);
        ...
        >
          <View style={styles.headerLeft}>
            <Text style={styles.semesterText}>
              {result.isCustom ? 'Custom Result' : `Semester ${result.semester}`}
            </Text>
